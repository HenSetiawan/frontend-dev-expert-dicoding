const itActsAsFavoriteCafeModel = (favoriteCafe) => {
  it('should return the cafe that has been added', async () => {
    ...
    expect(await favoriteCafe.getCafe(1)).toEqual({ id: 1 });
    expect(await favoriteCafe.getCafe(2)).toEqual({ id: 2 });
    expect(await favoriteCafe.getCafe(3)).toEqual(undefined);
  });
  ...
  it('should refuse a cafe from being added if it does not have the correct property', async () => {
    ...
    expect(await favoriteCafe.getAllCafes()).toEqual([]);
  });
